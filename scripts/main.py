    ...
    for i in range(n):
        ...
        for j in range(n):
            ...
            if(i < j):      
                ...
                r_ij = r[i][j]
            a_newtonian_i += M[j] / r_ij**3 * (x_i - x_j)     
            sum += M[j] / r_ij
...
if __name__ == "__main__":  
    ...
    t_span = [0, 3.154e7] # in s => 1 years
    # Step size 
    dt = 86400 # in seconds, 1 day
    timesteps = np.arange(t_span[0], t_span[1], dt)
    
    # Additional options for the ODE solver
    options = {'rtol': 1e-6, 'atol': 1e2}
    
    sol = ode.solve_ivp(f, t_span, y0, 'RK45', timesteps, args=(M,), **options)
